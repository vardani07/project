...
import { createNewMaterial, updateMaterial } from "../../context/materialsContext/apiCalls";
...
const ItemForm = ({
  ...
  const upload = (items) => {
    items.forEach((item) => {
      ...
      uploadBytes(storageRef, item)
        .then((snapshot) => {
          console.log(`${item.name} file successfully uploaded`);
        ...
        .catch((error) => {
          ...
          // Optionally, update UI to reflect upload failure
        });
    });
  };
  ...
  const handleUpload = ({ target: { files } }) => {
    if (files) {
      ...
      let filesData = Array.from(files);

      console.log("Files to upload:", filesData);
      upload(filesData);
    } else {
  ...
  const handleSubmit = (e) => {
    ...
    let item = { ...formdata };
    const { _id, course, semester } = subjects.find(
